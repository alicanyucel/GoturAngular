...
export class RegistryListService {
	...
	async getRegistryRequestList(query: IGetRegistryRequestListQuery): Promise<IGetRegistryRequestListQueryResponse> {
		return await firstValueFrom(this._httpClient.get<IGetRegistryRequestListQueryResponse>(`${environment.apiUrl}/api/Registry/CarrierRegistryRequests`, {
...
export interface IGetRegistryRequestListQueryResponse extends IPaginateResponse {
	data: Array<ICarrierRegistryRequest>;
}

export interface ICarrierRegistryRequest extends IBaseEntity<string> {
	name: string;
	surname: string;
